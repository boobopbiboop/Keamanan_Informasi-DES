def ascii_to_binary(text):
    return ''.join(format(ord(char), '08b') for char in text)

def binary_to_ascii(binary):
    return ''.join(chr(int(binary[i:i+8], 2)) for i in range(0, len(binary), 8))

def permute(block, table):
    return ''.join(block[i-1] for i in table)

def left_shift(bits, shifts):
    return bits[shifts:] + bits[:shifts]

def xor(a, b):
    return ''.join('0' if x == y else '1' for x, y in zip(a, b))

# DES Tables
IP = [58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4,
      62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8,
      57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3,
      61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7]

FP = [40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31,
      38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29,
      36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27,
      34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25]

PC1 = [57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18,
       10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36,
       63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22,
       14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4]

PC2 = [14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10,
       23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2,
       41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48,
       44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32]

E = [32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9,
     8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
     16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25,
     24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1]

P = [16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10,
     2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25]

S_BOXES = [
    [[14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
     [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
     [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
     [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]],
    
    [[15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
     [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
     [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
     [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]],
    
    [[10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
     [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
     [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
     [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]],
    
    [[7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
     [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
     [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
     [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]],
    
    [[2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
     [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
     [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
     [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]],
    
    [[12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
     [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
     [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
     [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]],
    
    [[4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
     [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
     [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
     [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]],
    
    [[13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
     [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
     [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
     [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]]
]

SHIFTS = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]

def s_box_substitution(expanded_half):
    result = ""
    for i in range(8):
        block = expanded_half[i*6:(i+1)*6]
        row = int(block[0] + block[5], 2)
        col = int(block[1:5], 2)
        result += format(S_BOXES[i][row][col], '04b')
    return result

def generate_keys(key_binary):
    print(f"Original Key: {key_binary}")
    
    # PC1 permutation
    key_56 = permute(key_binary, PC1)
    print(f"After PC1: {key_56}")
    
    left = key_56[:28]
    right = key_56[28:]
    
    keys = []
    for round_num in range(16):
        left = left_shift(left, SHIFTS[round_num])
        right = left_shift(right, SHIFTS[round_num])
        
        combined = left + right
        round_key = permute(combined, PC2)
        keys.append(round_key)
        
        print(f"Round {round_num + 1} Key: {round_key}")
    
    return keys

def des_round(left, right, round_key, round_num):
    print(f"\n--- Round {round_num} ---")
    print(f"Left: {left}")
    print(f"Right: {right}")
    print(f"Round Key: {round_key}")
    
    # Expansion
    expanded_right = permute(right, E)
    print(f"Expanded Right: {expanded_right}")
    
    # XOR with round key
    xor_result = xor(expanded_right, round_key)
    print(f"XOR Result: {xor_result}")
    
    # S-box substitution
    s_box_result = s_box_substitution(xor_result)
    print(f"S-box Result: {s_box_result}")
    
    # P permutation
    p_result = permute(s_box_result, P)
    print(f"P Permutation: {p_result}")
    
    # XOR with left half
    new_right = xor(left, p_result)
    print(f"New Right: {new_right}")
    
    return right, new_right

def des_process(text_binary, key, encrypt=True):
    mode = "ENCRYPTION" if encrypt else "DECRYPTION"
    print(f"=== DES {mode} ===")
    
    key_binary = ascii_to_binary(key)
    print(f"Key ASCII: {key}")
    print(f"Key Binary: {key_binary}")
    
    # Generate round keys
    round_keys = generate_keys(key_binary)
    if not encrypt:
        round_keys = round_keys[::-1]  # Reverse for decryption
    
    # Process each 64-bit block
    result = ""
    for i in range(0, len(text_binary), 64):
        block = text_binary[i:i+64]
        
        print(f"\n=== Processing Block {i//64 + 1} ===")
        print(f"Block: {block}")
        
        # Initial permutation
        ip_result = permute(block, IP)
        print(f"After IP: {ip_result}")
        
        left = ip_result[:32]
        right = ip_result[32:]
        
        # 16 rounds
        for round_num in range(16):
            left, right = des_round(left, right, round_keys[round_num], round_num + 1)
        
        # Final swap and permutation
        final_block = right + left
        print(f"Before FP: {final_block}")
        
        processed_block = permute(final_block, FP)
        print(f"After FP: {processed_block}")
        
        result += processed_block
    
    return result

def des_encrypt(plaintext, key):
    plaintext_binary = ascii_to_binary(plaintext)
    print(f"Plaintext ASCII: {plaintext}")
    print(f"Plaintext Binary: {plaintext_binary}")
    
    # Pad plaintext to 64 bits
    while len(plaintext_binary) % 64 != 0:
        plaintext_binary += '0'
    
    return des_process(plaintext_binary, key, encrypt=True)

def des_decrypt(ciphertext_hex, key):
    ciphertext_binary = bin(int(ciphertext_hex, 16))[2:].zfill(len(ciphertext_hex) * 4)
    print(f"Ciphertext Hex: {ciphertext_hex}")
    print(f"Ciphertext Binary: {ciphertext_binary}")
    
    decrypted_binary = des_process(ciphertext_binary, key, encrypt=False)
    
    # Remove padding and convert to ASCII
    decrypted_ascii = binary_to_ascii(decrypted_binary).rstrip('\x00')
    return decrypted_binary, decrypted_ascii

def main():
    while True:
        print("\n=== DES Step-by-Step Implementation ===")
        print("1. Encrypt")
        print("2. Decrypt")
        print("3. Exit")
        
        choice = input("Choose option: ")
        
        if choice == '1':
            plaintext = input("Enter plaintext (ASCII): ")
            key = input("Enter key (8 characters): ")
            
            if len(key) != 8:
                print("Key must be exactly 8 characters!")
                continue
            
            ciphertext = des_encrypt(plaintext, key)
            print(f"\nFinal Ciphertext (Binary): {ciphertext}")
            print(f"Final Ciphertext (Hex): {hex(int(ciphertext, 2))[2:].upper()}")
            
        elif choice == '2':
            ciphertext_hex = input("Enter ciphertext (Hex): ")
            key = input("Enter key (8 characters): ")
            
            if len(key) != 8:
                print("Key must be exactly 8 characters!")
                continue
            
            try:
                decrypted_binary, decrypted_ascii = des_decrypt(ciphertext_hex, key)
                print(f"\nDecrypted Binary: {decrypted_binary}")
                print(f"Decrypted ASCII: {decrypted_ascii}")
            except ValueError:
                print("Invalid hex format!")
            
        elif choice == '3':
            print("Exiting...")
            break
        else:
            print("Invalid option!")

if __name__ == "__main__":
    main()
